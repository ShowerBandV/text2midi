...
- ‚ú?You bring requirements extraction expertise
- ‚ú?User brings their product vision and context
...
- üíæ Populate {planning_artifacts}/epics.md with extracted requirements
...
**üö® CRITICAL**: Do NOT reduce UX requirements to vague summaries. Each UX-DR must be specific enough to generate a story with testable acceptance criteria. If the UX spec identifies 6 reusable components, list all 6 ‚Ä?not "create reusable components."
...
3. Replace placeholder sections with extracted requirements:
   - {{fr_list}} ‚Ü?extracted FRs
   - {{nfr_list}} ‚Ü?extracted NFRs
   - {{additional_requirements}} ‚Ü?extracted additional requirements (from Architecture)
   - {{ux_design_requirements}} ‚Ü?extracted UX Design Requirements (if UX document exists)
