...
- **File Churn Check:** Do multiple epics repeatedly modify the same core files?
  ...
  - ‚ù?WRONG: Multiple epics each modify the same core files with no feedback loop between them
  - ‚ú?RIGHT: Epics target distinct files/components, OR consolidation was explicitly considered and rejected with rationale
...
HALT ‚Ä?wait for user input before proceeding.
