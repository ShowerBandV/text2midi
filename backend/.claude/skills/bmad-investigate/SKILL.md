...
- **Challenge the premise.** The user's description is a hypothesis, not a fact. Verify independently; if evidence
  contradicts, say so.
- **Follow the evidence, not the narrative.** When evidence contradicts the working theory, update the theory â€?never
...
- **Issue independent operations in parallel** (multi-grep, multi-read, parallel inventories) â€?one message, multiple
  tool calls.
...
Acknowledge the input as a reference (record paths and IDs; don't read raw content). Path to an existing case file â†?Outcome 0. Otherwise â†?Outcome 1.
...
backlog (Status â‰?Done); missing-evidence rows; last Conclusion with confidence. Ask which thread to pull. New
...
Acknowledge each input shape â€?record location, scope, time window only; bulk reads happen in Outcome 2.
...
of: â€?; pause for the user to provide evidence or authorize Outcome 2 to scan more broadly.
...
Classify each Available, Partial, or Missing â€?Missing is itself a finding. Update Evidence Inventory and Investigation
...
crossings (compiledâ†’scripts, IPC, hostâ†’device, configuration flow).
...
- **Symptom-driven:** depth assessment â€?is the root cause reachable from local context, or is a broader area model
  required? Surface escalations; never silently expand scope. Trivial-fix assessment â€?off-by-one, missing null check,
  swapped argument â†?one-line code suggestion or draft diff in the report; non-trivial â†?stop at the root cause area.
...
Present the conclusion, then a concrete next-steps menu: trivial fix â†?`bmad-quick-dev`; scope/plan adjustment â†?`bmad-correct-course`; tracked story â†?`bmad-create-story`; fresh review â†?`bmad-code-review`. Recommend the
highest-value action. Mitigations and workarounds are generated only on explicit request â€?investigation stops at the
