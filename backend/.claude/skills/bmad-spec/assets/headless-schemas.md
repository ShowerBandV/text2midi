...
The default invocation is headless: input goes in, JSON comes out. The contract is intentionally tiny â€?return the outcome and the files touched. Anything else a caller needs is inside those files (SPEC.md, companions, `.decision-log.md`).
...
- `insufficient_intent` â€?input too thin to distill into a kernel.
- `missing_slug` â€?input is sparse or multi-source and no slug was provided by the caller or derivable from a source path.
