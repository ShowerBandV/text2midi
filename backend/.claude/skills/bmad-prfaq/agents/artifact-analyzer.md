...
You will receive:
- **Product intent:** A summary of the concept â€?customer, problem, solution direction
...
4. **Read all relevant documents in parallel** â€?issue all Read calls in a single message rather than one at a time. Extract:
   ...
   - Ideas, both accepted and rejected (rejected ideas are valuable â€?they prevent re-proposing)
...
Return ONLY the following JSON object. No preamble, no commentary. Keep total response under 1,500 tokens. Maximum 5 bullets per section â€?prioritize the most impactful findings.
...
```json
{
  ...
  "key_insights": [
    "bullet â€?grouped by theme, each self-contained"
  ],
  "user_market_context": [
    "bullet â€?users, market, competition found in docs"
  ],
  "technical_context": [
    "bullet â€?platforms, constraints, integrations"
  ],
  "ideas_and_decisions": [
    {"idea": "description", "status": "accepted|rejected|open", "rationale": "brief why"}
  ],
  "raw_detail_worth_preserving": [
    "bullet â€?specific details, data points, quotes for the distillate"
  ]
}
```
