...
<workflow>
    ...
    <check if="{{sprint_status}} file exists">
      <check if="{{current_status}} == 'ready-for-dev' OR (review_continuation == true AND {{current_status}} != 'in-progress')">
        ...
        <output>üöÄ Starting work on story {{story_key}}
          Status updated: {{current_status}} ‚Ü?in-progress
        </output>
      </check>
  ...
  <step n="5" goal="Implement task following red-green-refactor cycle">
    ...
    <action>Document technical approach and decisions in Dev Agent Record ‚Ü?Implementation Plan</action>
  ...
  <step n="8" goal="Validate and mark task complete ONLY when fully done">
    ...
    <check if="task is review follow-up (has [AI-Review] prefix)">
      ...
      <action>Mark task checkbox [x] in "Tasks/Subtasks ‚Ü?Review Follow-ups (AI)" section</action>

      <!-- CRITICAL: Also mark corresponding action item in review section -->
      <action>Find matching action item in "Senior Developer Review (AI) ‚Ü?Action Items" section by matching description</action>
      <action>Mark that action item checkbox [x] as resolved</action>

      <action>Add to Dev Agent Record ‚Ü?Completion Notes: "‚ú?Resolved review finding [{{severity}}]: {{description}}"</action>
    </check>
  ...
  <step n="9" goal="Story completion and mark for review" tag="sprint-status">
    ...
    <check if="{sprint_status} file exists AND {{current_sprint_status}} != 'no-sprint-tracking'">
      ...
      <output>‚ú?Story status updated to "review" in sprint-status.yaml</output>
    </check>
  ...
  <step n="10" goal="Completion communication and user support">
    ...
    <action>Prepare a concise summary in Dev Agent Record ‚Ü?Completion Notes</action>
