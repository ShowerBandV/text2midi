...
**Your Role:** You are a Developer providing clear, actionable sprint visibility. No time estimates â€?focus on status, risks, and next steps.
...
- Stories: everything else (e.g., 1-2-login-form)
  <action>Map legacy story status "drafted" â†?"ready-for-dev"</action>
  <action>Count story statuses: backlog, ready-for-dev, in-progress, review, done</action>
  <action>Map legacy epic status "contexted" â†?"in-progress"</action>
...
- Retrospectives: optional, done
  ...
  {{#each invalid_entries}}
  {{@index}}. {{key}}: "{{status}}" â†?[select valid status]
  {{/each}}
...
<step n="3" goal="Select next action recommendation">
  ...
  1. If any story status == in-progress â†?recommend `dev-story` for the first in-progress story
  2. Else if any story status == review â†?recommend `code-review` for the first review story
  3. Else if any story status == ready-for-dev â†?recommend `dev-story`
  4. Else if any story status == backlog â†?recommend `create-story`
  5. Else if any retrospective status == optional â†?recommend `retrospective`
  6. Else â†?All implementation items done; congratulate the user - you both did amazing work together!
