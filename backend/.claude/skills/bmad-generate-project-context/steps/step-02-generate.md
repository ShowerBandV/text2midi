...
- ‚ú?ALWAYS treat this as collaborative discovery between technical peers
...
- ‚ú?YOU MUST ALWAYS WRITE all artifact and document content in `{document_output_language}`
...
‚ú?All critical technology versions accurately documented
‚ú?Language-specific rules cover unobvious patterns
‚ú?Framework rules capture project-specific conventions
‚ú?Testing rules ensure consistent test quality
‚ú?Code quality rules maintain project standards
‚ú?Workflow rules prevent implementation conflicts
‚ú?Content is lean and optimized for LLM context
‚ú?A/P/C menu presented and handled correctly for each category

## FAILURE MODES:

‚ù?Including obvious rules that agents already know
‚ù?Making content too verbose for LLM context efficiency
‚ù?Missing critical anti-patterns or edge cases
‚ù?Not getting user validation for each rule category
‚ù?Not documenting exact versions and configurations
