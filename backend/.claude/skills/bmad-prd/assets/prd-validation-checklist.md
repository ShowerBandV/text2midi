...
A judgment rubric for the validator subagent. Walk the PRD with these dimensions in mind and write substantive findings â€?not box-ticking. The goal is a review that tells the user whether this PRD is *good*, not whether it has the right section headers.

Most PRDs do not need every dimension scrutinized equally. Calibrate to the agreed stakes, the PRD's shape (consumer product, internal tool, regulatory update, technical capability spec), and what the PRD itself is trying to do. Be specific â€?cite locations, quote phrases, name what's missing. Abstract criticism is failure of nerve.

## How to use this rubric

1. Read the full PRD (and addendum.md if present) before writing anything.
2. For each of the seven dimensions below, form a judgment â€?*strong / adequate / thin / broken* â€?backed by specifics from the PRD.
...
5. The overall verdict is your synthesis â€?2â€? sentences that name what holds up and what's at risk. Earn it with the dimension judgments.
...
```markdown
# PRD Quality Review â€?{prd_name}

## Overall verdict
[2â€? sentences. What holds up, what's at risk. Earned by the dimension judgments below.]

## Decision-readiness â€?[strong | adequate | thin | broken]
[1â€? paragraphs of judgment with specific PRD locations.]

### Findings
- **[critical|high|medium|low]** [Title] (Â§ location) â€?[Note]. *Fix:* [suggested fix].

## Substance over theater â€?[verdict]
...

(repeat for each dimension)

## Mechanical notes
[Glossary drift, ID continuity, broken cross-refs, Assumptions Index roundtrip. Lighter weight â€?these matter for downstream but don't drive the overall verdict.]
```
...
- Open Questions that are actually open â€?not rhetorical questions with an answer in the next sentence.
...
- **Persona theater** â€?Personas that don't drive a single decision in the PRD. More than four personas. Personas whose only function is to make the PRD look thorough.
- **Innovation theater** â€?claimed novelty that isn't novel. Differentiation sections written because the template had one, not because Discovery surfaced something.
- **NFR theater** â€?copied boilerplate ("system must be scalable / secure / reliable") without product-specific thresholds.
- **Vision theater** â€?a Vision statement that could swap into any PRD in this category without change.
...
- Feature prioritization that follows from the thesis â€?not from "what's easy first."
...
- Coherent MVP scope kind â€?problem-solving, experience, platform, or revenue â€?with scope logic that matches.
...
Look for:
- FRs with at least one testable consequence per FR â€?verifiable condition, measurable outcome.
- "System handles X gracefully," "reasonable performance," "user-friendly" â€?flag every one.
...
Look for:
- A Non-Goals section where it would do real work â€?and `[NON-GOAL for MVP]` callouts where omissions could be silently assumed.
- `[ASSUMPTION: â€¦]` tags on inferences the user didn't directly confirm, indexed at the end.
...
- Each section makes sense pulled out alone â€?cross-references via Glossary terms, not "see above."
- UJs each have a named protagonist; no floating UJs.

For standalone PRDs (no downstream), this dimension matters less â€?say so.

### 7. Shape fit

Has the PRD been forced into a shape that doesn't match the product?

- Consumer product / multi-stakeholder B2B / meaningful UX â†?UJs with named protagonists are load-bearing.
- Internal tool, single-operator role â†?capability spec shape; UJs may be overhead; SMs may be operational rather than user-facing.
- Regulatory or compliance update â†?constraint traceability is non-negotiable; UJs may be irrelevant.
- Hobby / solo â†?rigor light, substance bar still applies.
- Brownfield â†?existing-code references must be accurate; new UJs and existing UJs must be distinguished.
- Chain-top (feeds UX â†?architecture â†?stories) â†?downstream usability matters more; standalone PRDs can be lighter on traceability.
