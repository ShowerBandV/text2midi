...
Display: `[Orientation] 鈫?Walkthrough 鈫?Detail Pass 鈫?Testing`

## Follow Global Step Rules in SKILL.md

## FIND THE CHANGE

The conversation context before this skill was triggered IS your starting point 鈥?not a blank slate. Check in this order 鈥?stop as soon as the change is identified:

1. **Explicit argument**
   Did the user pass a PR, commit SHA, branch, or spec file this message?
   - PR reference 鈫?resolve to branch/commit via `gh pr view`. If resolution fails, ask for a SHA or branch.
   - Spec file, commit, or branch 鈫?use directly.
...
3. **Sprint tracking**
   ...
   - Exactly one 鈫?suggest it and confirm with the user.
   - Multiple 鈫?present as numbered options.
   - None 鈫?fall through.

4. **Current git state**
   Check current branch and HEAD. Confirm: "I see HEAD is `<short-sha>` on `<branch>` 鈥?is this the change you want to review?"
...
Set `change_type` to match how the user referred to the change 鈥?`PR`, `commit`, `branch`, or their own words (e.g. `auth refactor`). Default to `change` if ambiguous.

Set `review_mode` 鈥?pick the first match:

1. **`full-trail`** 鈥?ENRICH found a spec with a `## Suggested Review Order` section. Intent source: spec's Intent section.
2. **`spec-only`** 鈥?ENRICH found a spec but it has no Suggested Review Order. Intent source: spec's Intent section.
3. **`bare-commit`** 鈥?no spec found. Intent source: commit message. If the commit message is terse (under 10 words), scan the diff for the primary change pattern and draft a one-sentence intent. Flag it as `[inferred]` in the output so the user can correct it.

## PRODUCE ORIENTATION

### Intent Summary

- If intent comes from a spec's Intent section, display it verbatim regardless of length 鈥?it's already written to be concise.
- For other sources (commit messages, bug reports, user description): if 鈮?00 tokens, display verbatim. If longer, distill to 鈮?00 tokens. Link to the full source when one exists (e.g. a file path or URL).
...
3. `HEAD~1..HEAD` (latest commit only 鈥?tell the user).
...
```
N files changed 路 M modules touched 路 ~L lines of logic 路 B boundary crossings 路 P new public interfaces
```
...
```
[Orientation] 鈫?Walkthrough 鈫?Detail Pass 鈫?Testing
...
If review mode is not `full-trail`, read fully and follow `./generate-trail.md` to build one from the diff. Then return here and continue to NEXT. If trail generation fails (e.g., git unavailable), the original review mode is preserved 鈥?step-02 handles this with its non-trail path.
