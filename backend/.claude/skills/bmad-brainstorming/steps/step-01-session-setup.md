...
- ‚ú?ALWAYS treat this as collaborative facilitation
...
‚ú?Existing sessions detected without reading file contents
‚ú?User prompted to continue existing session or start new
‚ú?Correct session file selected for continuation
‚ú?Fresh workflow initialized with correct document structure
‚ú?Session context gathered and understood clearly
‚ú?User's approach selection captured and routed correctly
‚ú?Frontmatter properly updated with session state
‚ú?Document initialized with session overview section

## FAILURE MODES:

‚ù?Reading file contents during session detection (wastes context)
‚ù?Not asking user before continuing existing session
‚ù?Not properly routing user's continue/new session selection
‚ù?Missing continuation detection leading to duplicate work
‚ù?Insufficient session context gathering
‚ù?Not properly routing user's approach selection
‚ù?Frontmatter not updated with session parameters
