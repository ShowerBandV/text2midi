...
6. **Get answers to general questions** â€?when the question doesn't map to a specific skill, use the module's registered documentation to give a grounded answer
...
**Sequencing** determines recommended ordering within and across phases (these are soft suggestions, not hard gates â€?see `required` for gating):
- `preceded-by` â€?skills that should ideally complete before this one
- `followed-by` â€?skills that should ideally run after this one
...
- `[menu-code]` **Display name** â€?e.g., "[PR] PRD"
- Skill name in backticks â€?e.g., `bmad-prd`
