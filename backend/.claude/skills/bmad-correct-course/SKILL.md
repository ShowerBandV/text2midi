...
3. **Selectively load sections** based on relevance to the change being analyzed â€?do NOT load everything, only sections that relate to the impacted areas
4. **This document is optional** â€?skip if `{project_knowledge}` does not exist (greenfield projects)

**Fuzzy matching**: Be flexible with document names â€?users may use variations like `prd.md`, `bmm-prd.md`, `product-requirements.md`, etc.
...
<step n="1" goal="Initialize Change Navigation">
  ...
  <action>Verify access to project documents:</action>
    - PRD (Product Requirements Document) â€?required
    - Current Epics and Stories â€?required
    - Architecture documentation â€?optional, load if available
    - UI/UX specifications â€?optional, load if available
