...
**If the script fails**, resolve the `workflow` block yourself by reading these three files in base ‚Ü?team ‚Ü?user order and applying the same structural merge rules as the resolver:

1. `{skill-root}/customize.toml` ‚Ä?defaults
2. `{project-root}/_bmad/custom/{skill-name}.toml` ‚Ä?team overrides
3. `{project-root}/_bmad/custom/{skill-name}.user.toml` ‚Ä?personal overrides
...
Treat every entry in `{workflow.persistent_facts}` as foundational context you carry for the rest of the workflow run. Entries prefixed `file:` are paths or globs under `{project-root}` ‚Ä?load the referenced contents as facts. All other entries are facts verbatim.
...
<step n="1" goal="Determine target story">
  ...
  <check if="no user input provided">
    ...
    <check if="this is first story in epic {{epic_num}}">
      ...
      <action>If epic status is "backlog" ‚Ü?update to "in-progress"</action>
      <action>If epic status is "contexted" (legacy status) ‚Ü?update to "in-progress" (backward compatibility)</action>
      <action>If epic status is "in-progress" ‚Ü?no change needed</action>
...
<step n="2" goal="Load and analyze core artifacts">
  <critical>üî¨ EXHAUSTIVE ARTIFACT ANALYSIS - This is where you prevent future developer mistakes!</critical>
...
<step n="3" goal="Architecture analysis for developer guardrails">
  <critical>üèóÔ∏?ARCHITECTURE INTELLIGENCE - Extract everything the developer MUST follow!</critical> **ARCHITECTURE DOCUMENT ANALYSIS:** <action>Systematically
  ...
  <!-- Read existing code being modified ‚Ä?non-negotiable -->
  <critical>üìÇ READ FILES BEING MODIFIED ‚Ä?skipping this is the primary cause of implementation failures and review cycles</critical>
  ...
  <critical>A story implementation must leave the system working end-to-end ‚Ä?not just satisfy its stated ACs.
...
<step n="6" goal="Update sprint status and finalize">
  ...
  <action>Run: `python3 {project-root}/_bmad/scripts/resolve_customization.py --skill {skill-root} --key workflow.on_complete` ‚Ä?if the resolved value is non-empty, follow it as the final terminal instruction before exiting.</action>
</step>
