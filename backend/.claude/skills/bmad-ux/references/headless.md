...
Headless when any of: caller sets `headless: true` (or harness equivalent); invocation is from another skill or non-interactive runner; `{workflow.activation_steps_prepend}` declares it; first message is an automation context pre-supplying inputs. Ambiguous â†?default interactive.

## Inputs

Free-form structured payload in the first message:

- `intent` â€?`"create"`, `"update"`, or `"validate"`. If absent, infer from the artifact set.
- **Create**: any source spec (PRD, brief, requirements list, design-thinking output, prior UX â€?text, path, or URL) plus brand / platform / accessibility notes; `doc_workspace` if a specific run folder is required.
...
Inferences â†?`assumptions[]`. Gaps needing a human decision â†?`open_questions[]`. Do not invent persona, brand, accessibility, or scope detail.
...
`status`:
- `"complete"` â€?stands on its own.
- `"partial"` â€?artifact produced but `open_questions[]` non-empty or critical inputs inferred.
- `"blocked"` â€?no artifact produced.
