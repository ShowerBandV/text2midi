# DESIGN.md Spec â€?Working Reference
...
| `rounded` | object | Scale names (`sm`, `md`, `lg`, `xl`, `full`, `DEFAULT`) â†?CSS dimensions. `full` is conventionally `9999px`. |
| `spacing` | object | Scale levels (`'1'`, `'2'`, ...) or named tokens (`gutter`, `margin-mobile`, `editorial-gap`) â†?dimensions. |
| `components` | object | Component-name â†?object of component tokens mapped to values or `{path.to.token}` references. |

## Body sections (omittable, order-locked when present)

1. **Brand & Style** â€?Aesthetic posture in prose. The editorial voice â€?what *kind* of thing this is.
2. **Colors** â€?Per-color story. Why each exists, where it's used, what it's *not* used for.
3. **Typography** â€?Type roles, ramp, and rules. Platform conventions noted semantically when inherited.
4. **Layout & Spacing** â€?Spacing scale narrative, grid behavior, margins, gutters, breakpoint rules.
5. **Elevation & Depth** â€?Shadow language and tonal layering rules.
6. **Shapes** â€?Corner radii rules and the aesthetic logic behind them.
7. **Components** â€?Per-component visual specs: anatomy, color usage, sizing, state appearance.
8. **Do's and Don'ts** â€?Hard visual rules â€?what to do, what to avoid.
...
- **Platform conventions.** When inheriting from native platforms (iOS UIKit, Android Compose, Apple Human Interface Guidelines), use a `note` field instead of literal values: `{ note: 'iOS Title 1 Â· Android Headline Small' }`. The spec is the spec; the platform owns the rendered values.
