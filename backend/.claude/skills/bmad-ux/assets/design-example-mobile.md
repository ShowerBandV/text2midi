...
typography:
  title:
    note: 'Platform native â€?iOS Title 1 Â· Android Headline Small'
  body:
    note: 'Platform native â€?iOS Body Â· Android Body Large'
  meta:
    note: 'Platform native â€?iOS Footnote Â· Android Body Small'
rounded:
  sm: 6px
  md: 12px
spacing:
...
Quill is designed against the grain of contemporary habit apps. Where most products weaponize the user's calendar with streak counters and re-engagement nudges, Quill insists on something quieter â€?a daily prompt, a place to write, and the unspoken assurance that today's entry is enough. Showing up is the point, not the streak.
...
The palette is restrained on purpose â€?a writing surface should not compete with the writing.
...
- **Tobacco (`#A87434` light / `#D4A574` dark)** is the only chromatic color. Used exclusively for the save indicator and primary action â€?never for decoration, never for state badges.
...
Avoid: red error fills (Quill is a journal, not a form), gradients (the surface is paper), and saturated accent variants â€?one accent, used sparingly.

## Typography

Platform conventions are the spec. iOS uses Title 1 / Body / Footnote; Android uses Headline Small / Body Large / Body Small. Dynamic type honored at every level â€?the largest accessibility setting must still render legibly without truncation.
...
Quill avoids elevation as a visual device. Cards and composer surfaces sit on `surface-raised`, distinguished from `surface-base` only by tone. Shadows are reserved for the rare moment of literal physical metaphor â€?never for hierarchy. Hierarchy comes from layout and typography, not shadow.
...
- **Prompt card** â€?`surface-raised`. One per day. Today's prompt in `title`. Tap to open composer. No icon, no decoration; the prompt itself is the affordance.
- **Composer** â€?Full-screen text view. Clean text field, generous vertical padding, single-line save indicator in the header.
- **Save indicator** â€?Text only. Uses `ink-secondary`, never a checkmark icon, never a colored badge.
- **Entry row** (Library) â€?Date in `meta`, first line of body in `body` (truncated to one line). Hairline divider only, no fill.
- **Settings row** â€?Label left, value or chevron right. Tobacco accent only on destructive confirmations.
...
| Text-only state indicators (`Saved.`) | Iconography for state (âœ? âš? â—? |
