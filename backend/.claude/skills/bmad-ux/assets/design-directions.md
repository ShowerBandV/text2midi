...
Each direction is a *complete visual personality* applied to the same key screen â€?not a palette swap. Differ on density, type weight, motion implication, brand register. Each file: 2-3 sentence rationale, near-1:1 hero screen mockup in a phone or browser frame, ideally a secondary screen, at least one state variant visible (aging row, empty state, etc).

Use real product content from the conversation. Voice/tone from `.decision-log.md` applied to every visible string â€?no lorem. Inline CSS, system fonts, no JS or network. Document hex values in `<style>` comments per direction.
