  ...
  const handleGenerateMidi = async () => {
    ...
    try {
      ...
      if (data && data.notes) {
        ...
        setNotes(newTrack.notes);
        setTempo(newTrack.metadata.tempo);
        setRootKey(newTrack.metadata.key);
        setScaleType(newTrack.metadata.scale);

        // Auto transition into visual cassette library view so they look at it instantly
    ...
    <div className="min-h-screen bg-[#121212] flex flex-col font-sans text-on-background Selection:bg-primary-container Selection:text-on-primary-container">
      ...
      <main className="flex-grow pt-28 pb-xl">
        ...
        <div className="max-w-7xl mx-auto px-lg">
          ...
          {activeTab === "editor" && (
            <div className="h-[680px] border border-white/10 rounded-2xl overflow-hidden glass-panel flex flex-col shadow-2xl">
              <PianoRoll
                notes={notes}
                setNotes={setNotes}
                tempo={tempo}
